...
class Level extends LDTKLevel
{
	...
	function place_entities(project:LdtkProject, level_name:String)
	{
		...
		for (entity in data.l_Friendlies.all_Pod.iterator())
			new Pod(entity.pixelX, entity.pixelY);
		...
		for (entity in data.l_Enemies.all_Ghost.iterator())
			new Ghost(entity.pixelX, entity.pixelY);
